...
- `listar_dominios() returns setof text` — domínios de e-mail aceitos, para o front validar o cadastro com a mesma lista do banco.
...
**entrar.html** — duas abas: Entrar (e-mail, senha) e Cadastrar (nome, e-mail, senha). Cadastro valida o domínio no front (lista vinda de `listar_dominios()`, com a lista da PUC como reserva se a chamada falhar) antes de chamar `signUp` com `options.data.nome`. O parâmetro `voltar` só é aceito se for um nome de página relativo (sem `//` nem `://`). Se o projeto exigir confirmação de e-mail (`data.session` nulo), mostra "Confira seu e-mail". Depois de entrar, volta para `?voltar=` ou para o início.

**estudo.html?id=N** — chama `painel_estudo(N)`. Barra superior: voltar, título, disciplina, autor, botão de favoritar e dois botões de progresso (Estudando e Concluído; clicar no que já está ativo limpa o progresso), e para equipe "Marcar revisado" e "Excluir" (chama `excluir_estudo`, remove o arquivo do Storage com o caminho devolvido e volta ao início). Abaixo, o estudo em um `<iframe>` de altura `calc(100vh - barra)`: o JS faz `fetch(arquivo_url)`, reescreve no HTML todo `src="../` e `href="../` para a raiz absoluta do site (e o link `../index.html` ganha `target="_top"`), e carrega o resultado no iframe por uma URL `blob:` (`URL.createObjectURL`), já que as âncoras `href="#..."` dos estudos, dentro de um documento `about:srcdoc`, resolveriam contra a URL da página-mãe e navegariam o iframe em vez de rolar. Assim `../tema.css` e `../vendor/katex/...` continuam resolvendo, tanto para arquivo do repositório quanto para o Storage. Não se usa `<base>` porque os estudos têm dezenas de âncoras `href="#..."` que uma `<base>` faria apontar para fora do documento. Abaixo do iframe, comentários: lista (autor, selo do cargo, data, botão excluir quando `meu` ou equipe) e formulário (só logado; anônimo vê link para entrar).
